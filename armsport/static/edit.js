 ...
 */
function create_form(button,first_name,middle_name,last_name,age,weight,team){
    ...
    form.append('<input name="old_fn"/><input name="old_mn"/><input name="old_ln"/><input name="old_age"/><input name="old_weight"/><input name="old_team"/>' +
        ...
        '<button type="submit" name="edit" class="btn waves-effect waves-light green">Готово</button> ' +
        '</div>');
...
$('.edit').click(function () {
    ...
    console.log(list);
    var first_name = list[0].innerHTML.split(' ')[1];
    var middle_name = list[0].innerHTML.split(' ')[0];
    var last_name = list[0].innerHTML.split(' ')[2];
    var age = list[1].innerHTML.split(' ')[2];
    var weight = list[2].innerHTML.split(' ')[1].split('к')[0];
    var team = list[3].innerHTML.split(' ')[1];
    list.each(function () {
        $(this).remove();
    });
    console.log(first_name,middle_name,last_name,age,weight,team);
    create_form($(this),first_name,middle_name,last_name,age,weight,team);
    $(this).remove();
    ...
    $("#weight_edit").change(function () {
        ...
        if (weight_current[weight_current.length-1]=="+"){
            ...
            if (weight_prev) {
                ...
                weight_current = weight_current.split('к')[0];
                if ($(this).val() <= weight_current && temp.test($(this).val())) {
                    $(this).addClass('valid');
                }
                else {
